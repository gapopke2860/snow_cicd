        ...
        stage('Run schema change') {
            ...
            steps {
                ...
                withCredentials([aws(accessKeyVariable:'AWS_ACCESS_KEY_ID',credentialsId:'fdf98e8a-0ec7-4cb8-983a-856fbbb2d0c2',secretKeyVariable:'AWS_SECRET_ACCESS_KEY')]) {
                    ...
                    sh "python3 schemachange/schemachange/cli.py deploy -f migration -a ${SF_ACCOUNT} -u ${SF_USERNAME} -r ${SF_ROLE} -w ${SF_WAREHOUSE} -d ${SF_DATABASE} -c ${SF_DATABASE}.SCHEMACHANGE.CHANGE_HISTORY --create-change-history-table"
